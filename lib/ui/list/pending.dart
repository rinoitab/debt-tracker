// todo: Show all debt where isCompleted == false
...
class Pending extends StatelessWidget {
  ...
  Pending({this.payable});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text('Pending'),
          elevation: 0,),
        body: ListView.separated(
          ...
          itemBuilder: (context, index) {
            if (payable == null) {
              return Loading();
            } return pendingTile(context, index);
         },
        ),
    );
  }
  ...
  // todo: Show debt with isCompleted == false
  Widget pendingTile(BuildContext context, int index) {
